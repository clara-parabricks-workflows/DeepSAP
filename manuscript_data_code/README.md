...
6. **InDels analysis in Baruzzo datasets**: Refer to `Figure_6_InDels_plots_from_baruzzo_datasets/analyze_INDELs_baruzzo_datasets.html`
7. **Source data for figures**:  Refer to `figures_plots_and_data` folder and `source_data_for_figures.xlsx`
...
- Contains analysis of insertions and deletions (InDels) events.
- **Key folders and filess**:
  - `analyze_INDELs_baruzzo_datasets.html` - HTML report on InDels analysis.
  - `varscan_normalized_indels/` - Varscan processed results folder contains Varscan normalized indels for each aligner per dataset 
